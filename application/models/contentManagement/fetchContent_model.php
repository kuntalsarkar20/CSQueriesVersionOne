...
class fetchContent_model extends CI_Model{
	...
	public function homeCategory1Question(){	//gets 3 question to show in the home page of category DBMS
		$queryResult = $this->db->query('SELECT * FROM contents,category WHERE contents.CategoryId=1 AND category.CategoryId = contents.CategoryId order by views DESC,CreatedAt DESC Limit 3');
		return $queryResult->result_array();
	}
	public function homeCategory2Question(){	//gets 3 question to show in the home page of category DataStructure
		$queryResult = $this->db->query('SELECT * FROM contents,category WHERE contents.CategoryId=2 AND category.CategoryId = contents.CategoryId order by views DESC,CreatedAt DESC Limit 3');
		return $queryResult->result_array();
	}
